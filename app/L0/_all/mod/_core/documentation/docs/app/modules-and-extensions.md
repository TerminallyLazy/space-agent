...
- `_core/orchestrator` is the routed Orchestrator graph module: it mounts the infinite pan/zoom canvas at `#/orchestrator`, persists graph manifests and node files under `~/orchestrator/<graph-id>/` plus encrypted credential records under `~/orchestrator/secrets/`, exposes `space.orchestrator` for graph CRUD, edge mutation, topology application, agent runs, and Docker node controls, and delegates Docker socket access and SDK execution to backend `docker_*` and `agent_run_*` API endpoints; its local `AGENTS.md` owns the per-file ownership map, persistence contract, runtime namespace surface, and edge semantics
