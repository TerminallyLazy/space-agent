...
- `_core/orchestrator` is currently a model-only helper module: its local `AGENTS.md` owns the graph-model contract, and its JavaScript files define versioned graph constants, graph metadata normalization, node defaults and categories, edge type derivation, edge normalization, semantic edge colors, Bezier path helpers, canvas camera math, and graph-scoped message-bus routing without adding UI, storage, backend APIs, or a runtime namespace yet
