...
- `orchestrator_clients_test.mjs`: focused frontend-client coverage for `_core/orchestrator` Docker endpoint request shapes, agent-runner request shapes, encrypted credential storage and per-run decryption, and agent node adapter provider validation
