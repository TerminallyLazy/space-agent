...
- `orchestrator_model_test.mjs`: focused browser-model coverage for `_core/orchestrator` graph and node schema constants, node default normalization, node category rules, edge type derivation, edge normalization, semantic edge colors, canvas camera helpers, and graph-scoped message-bus routing
- `orchestrator_clients_test.mjs`: focused frontend-client coverage for `_core/orchestrator` Docker endpoint request shapes, agent-runner request shapes, encrypted credential storage and per-run decryption, agent node adapter provider validation, runtime namespace installation, and store-level `applyTopology` Docker network wiring
- `docker_service_test.mjs`: focused server-side coverage for the orchestrator Docker helper including `DOCKER_HOST` resolution, `_admin` mutation gating, normalized container summary shape, `createDockerService` list/start delegation, `docker_start` endpoint delegation, and `docker_exec` admin enforcement
- `agent_runner_service_test.mjs`: focused server-side coverage for the orchestrator agent runner service including provider/secret normalization, secret redaction, run record creation, in-memory status and event tracking, `agent_run_start` endpoint delegation, and `agent_run_events` endpoint readback
