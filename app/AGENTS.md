...
- `orchestrator/`: browser-side orchestration graph model helpers for versioned graph constants, graph metadata, node defaults, node categories, edge typing, edge normalization, canvas camera math, and graph-scoped message-bus routing
...
- `orchestrator/` owns the browser-side orchestration graph model, canvas camera helpers, and graph-scoped message-bus helpers; see `app/L0/_all/mod/_core/orchestrator/AGENTS.md`
