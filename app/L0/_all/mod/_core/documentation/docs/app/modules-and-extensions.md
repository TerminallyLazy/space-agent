...
- `_core/orchestrator` is a headless orchestration helper module: its local `AGENTS.md` owns the graph-model and storage contracts, and its JavaScript files define versioned graph constants, graph metadata normalization, node defaults and categories, edge type derivation, edge normalization, semantic edge colors, Bezier path helpers, canvas camera math, graph-scoped message-bus routing, and browser-owned YAML app-file storage under `~/orchestrator/` without adding UI, backend APIs, Docker clients, or a runtime namespace yet
