...
- `orchestrator/`: routed Orchestrator canvas, infinite pan/zoom graph editor for Docker container and AI agent nodes, persisted graphs under `~/orchestrator/`, encrypted credential records, frontend Docker and runner clients, and the `space.orchestrator` runtime namespace
...
- `orchestrator/` owns the routed Orchestrator canvas, graph and node persistence, the message bus, frontend Docker and runner clients, encrypted credential helpers, and the `space.orchestrator` runtime namespace; see `app/L0/_all/mod/_core/orchestrator/AGENTS.md`
